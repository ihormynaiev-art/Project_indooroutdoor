(function($) {
  "use strict";
    ...
    $(".add-extra").on('click', function () {
		...
		var servicecontent = '<div class="row service-cont justify-content-center">' +
			'<div class="col-md-4">' +
				'<div class="d-flex">' +
					'<div class="form-group w-100">' +
						'<label>Addional Email</label>' +
						'<input name="email[]" type="email" class="form-control" placeholder="Enter Service Duration">' +
					'</div>' +
...
$(function() {
  ...
  // Chat
    ...
    window.fileIdMap = new Map(); // Map для хранения соответствия ID -> File

    $("#icon-input, #image-input, #file-input").on('change', function (){
        const currentInput = $(this);
        const newFiles = Array.from(currentInput[0].files);

        // Добавляем новые файлы к накопленным
        window.accumulatedFiles = window.accumulatedFiles.concat(newFiles);

        // Обновляем input с объединенным списком файлов
        const dataTransfer = new DataTransfer();
        window.accumulatedFiles.forEach(file => {
            dataTransfer.items.add(file);
        });
        currentInput[0].files = dataTransfer.files;

        addFilesInfo(newFiles, currentInput)
    });
    ...
    if(isAdvancedUpload && draggableFileArea) {
        ...
        draggableFileArea.addEventListener("drop", e => {
            ...
            addFilesInfo(newFiles, $(fileInput))
        });
    }

    function addFilesInfo(files, inputElem){
        for (var i = 0; i < files.length; i++) {
            (function(file) {
                var fileId = 'new-' + newFileIndex;
                newFileIndex++;

                // Сохраняем соответствие ID -> File
                window.fileIdMap.set(fileId, file);

                // Только для #icon-input и #image-input показываем file-block, для #file-input - нет
                if (inputElem.attr('id') !== 'file-input') {
                    var html  =   `
                                    <div class="col-4 file-block mx-auto">
                                    ...
                                    `
                    inputElem.closest('.form-uploads-path').find(".file-block").remove();
                    inputElem.closest('.form-uploads-path').append(html);
                }
                    ...
                    let html  =   `
                               <li class="images-prio">
                                    <div class="img-preview">
                                        ...
                                        <a href="javascript:void(0);" data-id="${fileId}" data-name="${file.name}" class="remove-gallery"><i class="fa-solid fa-trash-can"></i></a>
                                    </div>
                                </li>
                                `
    ...
    $(".row").on("click", '.remove-gallery, .remove-file-icon', function (){
        var input = $(this).closest('.form-uploads-path').find('input[type="file"]')
        const fileArray = Array.from(input[0].files);
        const fileNameToRemove = $(this).data('name');
        const fileIdToRemove = $(this).data('id');
        ...
        // Обновляем глобальный массив накопленных файлов
        window.accumulatedFiles = window.accumulatedFiles.filter(file =>
            file.name.toLowerCase() !== fileNameToRemove.toLowerCase()
        );

        // Удаляем из Map по ID
        if (fileIdToRemove && window.fileIdMap) {
            window.fileIdMap.delete(fileIdToRemove);
        }
